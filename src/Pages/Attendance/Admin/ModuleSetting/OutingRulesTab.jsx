...
const RULE_CONFIGS = [
  {
    ruleCode: "outing_enabled",
    ruleName: "公出功能",
    valueType: "boolean",
    defaultValue: "1",
    description: "是否啟用公出申請相關功能。",
  },
  {
    ruleCode: "outing_form_description",
    ruleName: "公出申請說明",
    valueType: "text",
    defaultValue: "",
    description: "設定員工申請公出時顯示的說明文字。",
  },
];
...
export default function OutingRulesTab() {
  ...
  useEffect(() => {
    ...
    async function loadRules() {
      ...
      try {
        const response = await apiAttendanceOutingRules();
        const payload =
        ...
        console.error("Failed to load outing rules:", error);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await apiUpdateAttendanceOutingRule(
      ...
      console.error("Failed to update outing rule:", error);
  ...
  return (
    <Box>
      ...
      <FormDialog
        open={formOpen}
        title={activeConfig ? `編輯${activeConfig.ruleName}` : "公出規則"}
